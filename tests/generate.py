...
def main(run_id: str):
    dataset = YOLODataset(
        image_dir=FILE_PATH / ".." / "datasets" / "coco" / "images" / "val2017",
        label_dir=FILE_PATH / ".." / "datasets" / "coco" / "labels" / "val2017",
        max_labels=80,
        model_in_sz=(640, 640),
    ...
    for ep in tqdm(range(num_epochs), desc="Epochs"):
        patch, det_loss, nps_loss = optimization(model, hyp, dataloader, dataloader, patch, optimizer, device="cuda")
        if ep % val_step == 0:
